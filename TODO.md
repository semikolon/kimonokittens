...
11. **Font Rendering Investigation** (Nov 21, 2025) - User reports subtle kerning/spacing issue since woff2 extraction: "Galvji font seems horizontally a tiny bit compressed, or something, kerning/spacing could be the issue as well, but it's really hard to put a finger on. It is mostly relevant for the signup page, to be honest, because the only other situation that I need the web fonts for the DASHBOARD is when I view pop-os.local on my phone." Network requests confirmed both fonts loading correctly (Horsemen.woff2 + Galvji.ttc → 200 OK). Issue is comparing web font rendering vs. previous system font behavior. Low priority - mainly affects signup page and mobile dashboard viewing.
