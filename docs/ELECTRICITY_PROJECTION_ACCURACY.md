...
**Status**: 🚧 **In Progress** - Phase 1 infrastructure ready, VCR cassette generation next
**Completed**:
- ✅ VCR realistic test fixtures (Nov 20, 2025)
- ✅ Tibber data source clarification (Nov 20, 2025)
- ✅ Historical fixture extraction (64 bills, 2023-03 to 2025-10)
- ✅ Test spec with deterministic fixtures (Nov 20, 2025)
- ✅ Consumption data verification (~13 months rolling)
**Next**: VCR cassette generation for 12 testable periods, accuracy validation
...
|-----------|--------|------------------|-------|
| **Hourly consumption** | `electricity_usage.json` | Daily (3am cron) | **9,623 hours** (~13 months rolling), Sept 2024 → Nov 2025 |
| **Spot prices** | elprisetjustnu.se API | Real-time | SE3 region (Stockholm), **historical data available** ✅ |
...
**Consumption Data Details** (verified Nov 20, 2025):
- **Source**: `vattenfall.rb` scraper (lines 272-278)
- **Fetch logic**: `year - 1, month - 1` to `current month end`
- **Example**: If Nov 2025 → fetches Oct 20, 2024 through Nov 30, 2025
- **Coverage**: ~13 months of hourly data, sufficient for **all 12 testable periods**
- **Persistence**: File is overwritten daily, but 13-month window means we can test recent year
...
**Status**: ✅ Infrastructure complete, ⏳ VCR cassette generation pending

**Completed (Nov 20, 2025)**:
1. ✅ Extraction script: `bin/extract_historical_test_data.rb`
2. ✅ Historical fixture: `spec/fixtures/historical_electricity_bills.json` (64 bills, 32 periods)
3. ✅ Test spec: `spec/electricity_projector_historical_accuracy_spec.rb` (deterministic, fixture-based)
4. ✅ Consumption data verified: 9,623 hours, 13-month rolling window
5. ✅ Spot price API verified: Historical data available via elprisetjustnu.se

**Implementation Details**:
- **Fixture approach**: Tests load from JSON (not database), ensuring determinism
- **Testable periods**: 12 months filtered from fixture (last 12 months where consumption data exists)
- **Test database independence**: No historical data required in test DB

---

#### 1.1 Data Preparation Strategy ✅ COMPLETE

**Historical bills source**: JSON fixture from development database
- **Fixture**: `spec/fixtures/historical_electricity_bills.json`
- **Generated by**: `bin/extract_historical_test_data.rb` (queries dev DB)
- **Coverage**: 64 bills (32 periods), 2023-03 through 2025-10
- **Providers**: Vattenfall (31 bills) + Fortum (33 bills)
- **Approach**: Deterministic fixture (not database-dependent), version-controlled

**Consumption data source**: `electricity_usage.json` (verified Nov 20, 2025)
- **Current size**: 9,623 hourly data points (~13 months)
- **Date range**: Sept 26, 2024 → Nov 1, 2025
- **Scraper**: `vattenfall.rb` (lines 272-278), runs daily at 3am
- **Fetch window**: `(year-1, month-1)` to `(year, month, -1)` = ~13 months rolling
- **Sufficient for**: All 12 testable periods in fixture
...
#### 1.2 Test Data Extraction Script ✅ COMPLETE

**Purpose**: Query ElectricityBill database and aggregate by config period.

**Implemented**: `bin/extract_historical_test_data.rb` (Nov 20, 2025)
```ruby
#!/usr/bin/env ruby
require 'dotenv/load'
require 'date'
...
#### 1.3 Historical Validation Spec ✅ COMPLETE

**File**: `spec/electricity_projector_historical_accuracy_spec.rb` (Nov 20, 2025)

**Purpose**: Compare projector outputs to actual aggregated bills.

**Key features**:
- Loads historical data from JSON fixture (deterministic)
- Filters to 12 testable periods (last 12 months)
- Individual period accuracy tests (10% error threshold)
- Aggregate metrics: MAPE (<8%), MAE (<200 kr), bias (<3%)
- Seasonal validation (winter 30%+ more than summer)
...
#### 1.4 VCR Cassette Generation Strategy ⏳ NEXT STEP

**Status**: ✅ API verified to have historical data (Nov 20, 2025)

**Verification**:
```bash
# Test confirmed: Historical spot prices available
curl "https://www.elprisetjustnu.se/api/v1/prices/2024/11-01_SE3.json"
# Returns: 24 hours of real November 2024 pricing data
```

**Implementation approach**: VCR `:once` mode with real API calls
1. **First test run**: VCR captures real API responses for each period
2. **Cassette storage**: `spec/fixtures/vcr_cassettes/electricity_prices_YYYY_MM.yml`
3. **Subsequent runs**: Replays cassettes (no network, deterministic)
4. **Commit cassettes**: Version control ensures test reproducibility

**Cassettes needed**: 12 files (one per testable period in last 12 months)
- Example: `electricity_prices_2024_12.yml`, `electricity_prices_2025_01.yml`, etc.
- Each cassette: ~30 days × 24 hours = 720 API calls = ~150KB per file

**Next action**: Run spec once to capture all cassettes, then commit to git
