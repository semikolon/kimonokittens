...
function DashboardContent() {
  ...
  // Update page title based on current view mode
  useEffect(() => {
    document.title = viewMode === 'admin'
      ? 'Kimono Kittens: Admin'
      : 'Kimono Kittens: Dashboard'
  }, [viewMode])

  // Pause CSS animations ONLY when fully asleep
  const shouldPauseAnimations = sleepState.currentState === 'sleeping';

  // Hide gradient blobs in dev mode (less visual noise during development)
  const showGradients = import.meta.env.PROD;
  ...
  if (viewMode === 'admin') {
    return (
      <div className="min-h-screen w-full bg-[radial-gradient(circle_at_center,_rgb(28,22,35)_0%,_rgb(25,18,32)_100%)] overflow-x-clip relative">
        ...
        {showGradients && (
          <div
            className="gradients-container fixed inset-0 h-full w-full opacity-[0.37] blur-[50px] pointer-events-none"
            style={{
              zIndex: 2,
